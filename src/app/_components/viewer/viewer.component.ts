...
export class ViewerComponent implements OnInit {
    ...
    public async ngOnInit(): Promise<void> {
        ...
        client.on("update", (universe) => {
            /* universe.forEach((channelVal, channelNr) => {
              this.fixtures.forEach((f) => {
                ...
                if (f.startAddress <= channelNr
                    && f.startAddress + totalChannelLength - 1 >= channelNr) {
                  f.channels.forEach((c: Channel) => {
                    if (f.startAddress + c.startAddress - 1 <= channelNr &&
                      ...
                      // if (value != 0)
                      console.log("found fixture", f.displayName, "with", c.name);
                      if (c.type == "Steps") {
                        c.steps.forEach((step, idx) => {
                          if (step.start <= channelVal && (!c.steps[idx + 1]
                            || c.steps[idx + 1].start > channelVal)) {
                            if (this.colorNameToHex(step.name) == "rainbow") {
                              ...
                              f.object.rainbowInterval = setInterval(() => {
                                ...
                                if (f.object.currentRainbowColorIndex
                                    == that.rainbowColors.length) {
                                  f.object.currentRainbowColorIndex = 0;
                                }
                                f.object.fixtureLight.material =
                                  that.createMaterial(
                                    that.color3FromHex(
                                      that.colorNameToHex(
                                        that.rainbowColors[f.object.currentRainbowColorIndex]),
                                    ),
        ...
        ipcRenderer.on("getFixtures", (e, a) => {
            ...
            this.createScene();
            this.camera.position = new Vector3(this.numberOfFixtures
                * this.fixtureSpace * 0.5, 60, -200);
    ...
    private replaceDarkOrLight(name: string): string {
